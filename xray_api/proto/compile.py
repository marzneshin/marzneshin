        ...
        for file in files:
            ...
            if file.endswith(".py"):
                ...
                with open(path, 'r') as pyfile:
                    content = pyfile.read()
                    for imp in imports:
                        content = re.sub(fr"^([^'\r\n]*){imp}([\s\.].*)$", fr"\1{parent_dir.name}.{curr_dir.name}.{imp}\2", content, flags=re.MULTILINE)
                        content = re.sub(fr"^(.*importlib\.import_module\(\'){imp}(.*)$", fr"\1{parent_dir.name}.{curr_dir.name}.{imp}\2", content, flags=re.MULTILINE)
