...
def upgrade() -> None:
    ...
    sa.Column('is_otp_enabled', sa.Boolean(), server_default=sa.text('0'), nullable=False),
    ...
    sa.Column('status', sa.Enum('unhealthy', 'healthy', name='nodestatus'), nullable=False),
    ...
    op.create_index(op.f('ix_backends_node_id'), 'backends', ['node_id'], unique=False)

    op.create_table('inbounds',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('protocol', sa.Enum('vless', 'vmess', 'trojan', 'shadowsocks', 'socks', 'http', 'hysteria2', 'hysteria', 'tuic', 'shadowtls', 'shadowsocks2022', name='proxytypes'), nullable=True),
