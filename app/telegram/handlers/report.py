    ...
    if bot and (TELEGRAM_ADMIN_ID or TELEGRAM_LOGGER_CHANNEL_ID):
        try:
            if TELEGRAM_LOGGER_CHANNEL_ID:
                await bot.send_message(TELEGRAM_LOGGER_CHANNEL_ID, message, parse_mode=parse_mode)
            else:
                for admin in TELEGRAM_ADMIN_ID:
                    await bot.send_message(admin, message, parse_mode=parse_mode, reply_markup=keyboard)
        except ApiTelegramException as e:
            logger.error(e)


async def report_new_user(user_id: int, username: str, by: str, expire_date: int, data_limit: int, services: list):
    text = '''\
🆕 <b>#Created</b>
➖➖➖➖➖➖➖➖➖
...
async def report_user_modification(username: str, expire_date: int, data_limit: int, services: list, by: str):
    text = '''\
✏️ <b>#Modified</b>
➖➖➖➖➖➖➖➖➖
<b>Username :</b> <code>{username}</code>
...
<b>By :</b> <b>#{by}</b>\
    ...
    return await report(text, keyboard=BotKeyboard.user_menu({
        'username': username,
        'status': 'active'
    }, with_back=False))
