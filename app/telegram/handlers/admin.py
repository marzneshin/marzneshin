...
from app.telegram.utils.custom_filters import query_equals, query_startswith
...
async def delete_user_command(call: types.CallbackQuery):
    ...
    await bot.edit_message_text(
        ...
        call.message.message_id,
        parse_mode="markdown",
        reply_markup=BotKeyboard.confirm_action(action="do_delete", username=username),
    )
...
async def suspend_user_command(call: types.CallbackQuery):
    ...
    await bot.edit_message_text(
        ...
        call.message.message_id,
        parse_mode="markdown",
        reply_markup=BotKeyboard.confirm_action(action="do_suspend", username=username),
    )
...
async def edit_user_command(call: types.CallbackQuery):
    ...
    if action == "data":
        ...
        await bot.delete_state(call.from_user.id, call.message.chat.id)
        await bot.set_state(
            call.from_user.id, botStates.edit_user_data_limit, call.message.chat.id
        )
        ...
        # bot.clear_step_handler_by_chat_id(call.message.chat.id)
        await bot.set_state(
            call.from_user.id, botStates.edit_user_expire, call.message.chat.id
        )
        async with bot.retrieve_data(call.from_user.id, call.message.chat.id) as data:
            data["username"] = username
        # bot.register_next_step_handler(
...
async def edit_user_data_limit_step(message: types.Message):
    try:
        ...
        wait_msg = await bot.send_message(
            message.chat.id, "❌ Data limit must be a number."
        )
        schedule_delete_message(message.chat.id, wait_msg.message_id)
        return  # bot.register_next_step_handler(wait_msg, edit_user_data_limit_step, username=username)
...
async def edit_note_step(message: types.Message):
    ...
    with GetDB() as db:
        ...
        db_user = crud.update_user(
            db, db_user, UserModify(username=username, note=note)
        )
        user = UserResponse.model_validate(db_user)
    ...
    await bot.reply_to(
        ...
        reply_markup=BotKeyboard.user_menu(
            user_info={"status": user.status, "username": user.username}, note=True
        ),
    )
...
async def user_command(call: types.CallbackQuery):
    ...
    await bot.edit_message_text(
        ...
        parse_mode="HTML",
        reply_markup=BotKeyboard.user_menu(
            {"username": user.username, "status": user.status}, page=page, note=True
        ),
    )
...
async def random_username(call: types.CallbackQuery):
    ...
    schedule_delete_message(call.message.chat.id, msg.id)
    await bot.set_state(
        call.from_user.id, botStates.add_user_data_limit, call.message.chat.id
    )
    async with bot.retrieve_data(call.from_user.id, call.message.chat.id) as data:
        data["username"] = username
    return
...
async def add_user_command(call: types.CallbackQuery):
    ...
    await bot.set_state(
        call.from_user.id, botStates.add_user_username, call.message.chat.id
    )
...
async def add_user_username_step(message: types.Message):
    ...
    if not username:
        wait_msg = await bot.send_message(
            message.chat.id, "❌ Username can not be empty."
        )
        schedule_delete_message(message.chat.id, wait_msg.id)
        schedule_delete_message(message.chat.id, message.id)
    ...
    with GetDB() as db:
        if crud.get_user(db, username):
            wait_msg = await bot.send_message(
                message.chat.id, "❌ Username already exists."
            )
            schedule_delete_message(message.chat.id, wait_msg.id)
            schedule_delete_message(message.chat.id, message.id)
            return  # bot.register_next_step_handler(wait_msg, add_user_username_step)
    ...
    schedule_delete_message(message.chat.id, msg.id)
    await bot.set_state(
        message.from_user.id, botStates.add_user_data_limit, message.chat.id
    )
...
async def add_user_data_limit_step(message: types.Message):
    ...
    schedule_delete_message(message.chat.id, msg.id)
    await bot.set_state(
        message.from_user.id, botStates.add_user_expire, message.chat.id
    )
    async with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
...
async def disable_user(call: types.CallbackQuery):
    ...
    with GetDB() as db:
        db_user = crud.get_user(db, username)
        crud.update_user(
            db, db_user, UserModify(username=username, status=UserStatusModify.disabled)
        )
        await xray.operations.remove_user(db_user)
        user = UserResponse.model_validate(db_user)
    await bot.edit_message_text(
        ...
        parse_mode="HTML",
        reply_markup=BotKeyboard.user_menu(
            user_info={"status": "disabled", "username": db_user.username}, note=True
        ),
    )
    if TELEGRAM_LOGGER_CHANNEL_ID:
        text = f"""\
...
async def activate_user(call: types.CallbackQuery):
    ...
    with GetDB() as db:
        db_user = crud.get_user(db, username)
        crud.update_user(
            db, db_user, UserModify(username=username, status=UserStatusModify.active)
        )
    ...
    await bot.edit_message_text(
        ...
        parse_mode="HTML",
        reply_markup=BotKeyboard.user_menu(
            user_info={"status": "active", "username": db_user.username}, note=True
        ),
    )
    if TELEGRAM_LOGGER_CHANNEL_ID:
        text = f"""\
...
async def edit_user(call: types.CallbackQuery):
    ...
    if not mem_store.get(f"{call.message.chat.id}:services"):
        return await bot.answer_callback_query(
            call.id, "❌ No Service selected.", show_alert=True
        )
    ...
    with GetDB() as db:
        ...
        modify = UserModify(
            ...
            expire=(
                int(mem_store.get(f"{call.message.chat.id}:expire_date").timestamp())
    ...
    await bot.edit_message_text(
        ...
        parse_mode="HTML",
        reply_markup=BotKeyboard.user_menu(
            {"username": db_user.username, "status": db_user.status}, note=True
        ),
    )
    if TELEGRAM_LOGGER_CHANNEL_ID:
        tag = f'\n➖➖➖➖➖➖➖➖➖ \n<b>By :</b> <a href="tg://user?id={call.from_user.id}">{call.from_user.full_name}</a>'
...
async def delete_limpired(call: types.CallbackQuery):
    ...
    if not mem_store.get(f"{call.message.chat.id}:services"):
        return await bot.answer_callback_query(
            call.id, "❌ No services selected.", show_alert=True
        )
    ...
    new_user = UserCreate(
        ...
        data_limit=(
            mem_store.get(f"{call.message.chat.id}:data_limit")
...
@bot.callback_query_handler(
    query_startswith(["do_delete_expired", "do_delete_limited"]), is_admin=True
)
async def delete_limpired(call: types.CallbackQuery):
    await bot.edit_message_text(
        "⏳ <b>In Progress...</b>",
    ...
    with GetDB() as db:
        depleted_users = crud.get_users(
            db,
            status=[
                UserStatus.limited if data == "delete_limited" else UserStatus.expired
            ],
        )
                ...
                try:
                    if user.data_limit and user.status not in [
                        ...
                        user = crud.update_user(
                            ...
                            UserModify(
                                username=user.username,
                                data_limit=(user.data_limit + data_limit),
                            ),
                        )
                        counter += 1
                        f.write(
