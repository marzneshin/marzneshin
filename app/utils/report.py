...
async def data_usage_percent_reached(
    ...
    await notify(ReachedUsagePercent(username=user.username, user=user, used_percent=percent))
    await create_notification_reminder(db, ReminderType.data_usage,
                                       expires_at=dt.utcfromtimestamp(expire) if expire else None, user_id=user_id)


async def expire_days_reached(db: Session, days: int, user: UserResponse, user_id: int, expire: int) -> None:
