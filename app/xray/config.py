...
from app.models.proxy import ProxyTypes, InboundBase
...
class XRayConfig(dict):
    ...
    def include_db_users(self, node_id: int) -> XRayConfig:
        config = self.copy()

        with GetDB() as db:
            inbounds = [InboundBase(protocol=i["protocol"], tag=i["tag"]) for i in self.inbounds]
            crud.assure_node_inbounds(db, 
                                      inbounds=inbounds, node_id=node_id)
            for user_id, username, user_key, inbound_protocol, inbound_tag in crud.get_node_users(db, statuses=[UserStatus.active,
                                                                                UserStatus.on_hold], node_id=node_id):
                config.add_inbound_client(inbound_tag, f"{user_id}.{username}", generate_settings(user_key, inbound_protocol))
        if DEBUG:
