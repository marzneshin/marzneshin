...
async def send(data: List[Dict[Any, Any]]) -> bool:
    ...
    try:
        ...
        r = await aiohttp.request("post", config.WEBHOOK_ADDRESS, json=data, headers=headers)
        if r.ok:
            return True
        logger.error(r)
...
async def send_notifications():
    ...
    if not notifications_to_send:
        return
    if not await send([jsonable_encoder(notif) for notif in notifications_to_send]):
