...
async def get_users_stats(api: XRayAPI):
    try:
        params = defaultdict(int)
        for stat in filter(attrgetter('value'), await api.get_users_stats(reset=True)):
...
async def get_outbounds_stats(api: XRayAPI):
    try:
        params = [{"up": stat.value, "down": 0} if stat.link == "uplink" else {"up": 0, "down": stat.value}
                  for stat in filter(attrgetter('value'), await api.get_outbounds_stats(reset=True))]
        return params
    except (xray_exc.ConnectionError, xray_exc.UnknownError):
        return []
...
async def record_user_usages():
    ...
    for node_id, node in list(xray.nodes.items()):
        if node.started and await node.is_healthy():
            api_instances.append((node_id, node.api))
            usage_coefficient[node_id] = node.usage_coefficient  # fetch the usage coefficient

    results = await asyncio.gather(
             *[get_users_stats(api[1]) for api in api_instances]
             )
    api_params = {api_instances[i][0]: results[i] for i in range(len(api_instances))}
...
async def record_node_usages():
    ...
    for node_id, node in list(xray.nodes.items()):
        if node.started and await node.is_healthy():
            api_instances.append((node_id, node.api))
    results = await asyncio.gather(
            *[get_outbounds_stats(api[1]) for api in api_instances]
            )
    api_params = {api_instances[i][0]: results[i] for i in range(len(api_instances))}
